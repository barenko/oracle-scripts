...
set serveroutput on;
DECLARE
  TABELA_RAIZ VARCHAR(32);
  TABELA_FOLHA VARCHAR(32);
	...
	END BUSCA_EM_NIVEL;
BEGIN
  TABELA_RAIZ := '&TABELA_RAIZ';
  TABELA_FOLHA := '&TABELA_FOLHA';

  dbms_output.enable(200000);

  IF TABELA_RAIZ is null THEN
     BUSCA_EM_NIVEL(TABELA_RAIZ,TABELA_FOLHA,2,3);
  ELSE
     FOR I IN (SELECT TABLE_NAME FROM ALL_TABLES
         where table_name not in (select table_name from all_constraints where constraint_type = 'R')
     ) LOOP
